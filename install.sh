#!/bin/bash

GREEN='\033[0;32m'
CYAN='\033[0;36m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
NC='\033[0m' # No Color


clear

echo -e "${CYAN}***************************************************${NC}"
echo -e "${CYAN}* *${NC}"
echo -e "${CYAN}* MAHDI - VPN MANAGER SETUP         *${NC}"
echo -e "${CYAN}* *${NC}"
echo -e "${CYAN}***************************************************${NC}"
echo -e "${YELLOW}   >>> Starting Installation <<<   ${NC}"
echo ""

# 1. چک کردن دسترسی روت
if [ "$EUID" -ne 0 ]; then 
    echo -e "${RED}❌ لطفاً با دسترسی روت اجرا کنید (sudo su)${NC}"
    exit
fi

# 2. دریافت توکن و آیدی
echo -e "${GREEN}[?] Please enter your Telegram Bot Token:${NC}"
read -p "Token: " BOT_TOKEN

echo -e "${GREEN}[?] Please enter your Admin Numeric ID:${NC}"
read -p "Admin ID: " ADMIN_ID

if [[ -z "$BOT_TOKEN" || -z "$ADMIN_ID" ]]; then
    echo -e "${RED}❌ Error: Token and Admin ID cannot be empty!${NC}"
    exit 1
fi

# 3. نصب پیش‌نیازها
echo -e "${CYAN}[1/8] Updating System & Installing Dependencies...${NC}"
apt update -q -y
apt install -q -y wget curl nano python3 python3-pip unzip iptables

# نصب کتابخانه‌های پایتون (رفع ارور سیستم‌های جدید)
echo -e "${CYAN}[2/8] Installing Python Libraries...${NC}"
rm /usr/lib/python3.*/EXTERNALLY-MANAGED 2>/dev/null
pip3 install pyTelegramBotAPI requests pysocks --break-system-packages 2>/dev/null || pip3 install pyTelegramBotAPI requests pysocks

# 4. دانلود و نصب هسته سایفون
echo -e "${CYAN}[3/8] Installing Psiphon Core...${NC}"
wget -qO /usr/local/bin/psiphon https://github.com/Psiphon-Labs/psiphon-tunnel-core-binaries/raw/master/linux/psiphon-tunnel-core-x86_64
chmod +x /usr/local/bin/psiphon

# 5. ساخت ساختار پوشه‌ها برای مدیریت چند لوکیشن
echo -e "${CYAN}[4/8] Preparing multi-location structure...${NC}"
BASE_DIR="/etc/vpn-manager"
CONF_DIR="$BASE_DIR/configs"
DATA_DIR="$BASE_DIR/data"
mkdir -p "$CONF_DIR" "$DATA_DIR"

SIG_KEY=""

cat > "$BASE_DIR/sig.key" <<EOF
$SIG_KEY
EOF

# 6. ساخت helper.py برای مدیریت دیتابیس لوکیشن‌ها (JSON)
echo -e "${CYAN}[5/8] Creating location database helper...${NC}"
cat > "$BASE_DIR/helper.py" << 'PYEOF'
import json
import sys
import os

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_FILE = os.path.join(BASE_DIR, "locations.json")

def load_db():
    if not os.path.exists(DB_FILE):
        return {"locations": [], "next_id": 1}
    with open(DB_FILE, "r") as f:
        return json.load(f)

def save_db(db):
    with open(DB_FILE, "w") as f:
        json.dump(db, f, indent=4)

def cmd_add(name, region, port):
    db = load_db()
    port = int(port)
    for loc in db["locations"]:
        if loc["port"] == port:
            print(f"ERROR: Port {port} already in use by location '{loc['name']}'", file=sys.stderr)
            sys.exit(1)
    loc_id = f"loc{db['next_id']}"
    db["next_id"] += 1
    entry = {"id": loc_id, "name": name, "region": region, "port": port}
    db["locations"].append(entry)
    save_db(db)
    print(json.dumps(entry))

def cmd_remove(loc_id):
    db = load_db()
    before = len(db["locations"])
    db["locations"] = [l for l in db["locations"] if l["id"] != loc_id]
    if len(db["locations"]) == before:
        print(f"ERROR: Location '{loc_id}' not found", file=sys.stderr)
        sys.exit(1)
    save_db(db)
    print("OK")

def cmd_list():
    db = load_db()
    print(json.dumps(db["locations"]))

def cmd_get(loc_id):
    db = load_db()
    for loc in db["locations"]:
        if loc["id"] == loc_id:
            print(json.dumps(loc))
            return
    print(f"ERROR: Location '{loc_id}' not found", file=sys.stderr)
    sys.exit(1)

def cmd_update_region(loc_id, region):
    db = load_db()
    for loc in db["locations"]:
        if loc["id"] == loc_id:
            loc["region"] = region
            save_db(db)
            print("OK")
            return
    print(f"ERROR: Location '{loc_id}' not found", file=sys.stderr)
    sys.exit(1)

def cmd_update_name(loc_id, name):
    db = load_db()
    for loc in db["locations"]:
        if loc["id"] == loc_id:
            loc["name"] = name
            save_db(db)
            print("OK")
            return
    print(f"ERROR: Location '{loc_id}' not found", file=sys.stderr)
    sys.exit(1)

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: helper.py <add|remove|list|get|update_region|update_name> [args]", file=sys.stderr)
        sys.exit(1)
    cmd = sys.argv[1]
    args = sys.argv[2:]
    try:
        if cmd == "add":
            cmd_add(*args)
        elif cmd == "remove":
            cmd_remove(*args)
        elif cmd == "list":
            cmd_list()
        elif cmd == "get":
            cmd_get(*args)
        elif cmd == "update_region":
            cmd_update_region(*args)
        elif cmd == "update_name":
            cmd_update_name(*args)
        else:
            print(f"ERROR: Unknown command '{cmd}'", file=sys.stderr)
            sys.exit(1)
    except TypeError:
        print("ERROR: Invalid arguments", file=sys.stderr)
        sys.exit(1)
PYEOF

# ساخت traffic_helper.py برای نگهداری آمار ترافیک پایدار (باقی می‌ماند بعد از ریبوت سرور)
cat > "$BASE_DIR/traffic_helper.py" << 'TPYEOF'
import json
import sys
import os

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
TRAFFIC_FILE = os.path.join(BASE_DIR, "traffic.json")

def load_db():
    if not os.path.exists(TRAFFIC_FILE):
        return {}
    try:
        with open(TRAFFIC_FILE, "r") as f:
            return json.load(f)
    except Exception:
        return {}

def save_db(db):
    with open(TRAFFIC_FILE, "w") as f:
        json.dump(db, f, indent=2)

def cmd_update(loc_id, cur_up, cur_down):
    # cur_up/cur_down = شمارنده خام لحظه‌ای kernel (iptables) که ممکن است بعد از ریبوت صفر شده باشد
    cur_up = int(cur_up)
    cur_down = int(cur_down)
    db = load_db()
    st = db.get(loc_id, {"base_up": 0, "base_down": 0, "last_up": 0, "last_down": 0})

    if cur_up >= st["last_up"]:
        delta_up = cur_up - st["last_up"]
    else:
        # شمارنده کمتر از قبل شده یعنی سرور ریبوت شده و kernel counter صفر شده؛
        # کل مقدار فعلی، رشد تازه بعد از ریست است
        delta_up = cur_up

    if cur_down >= st["last_down"]:
        delta_down = cur_down - st["last_down"]
    else:
        delta_down = cur_down

    st["base_up"] += delta_up
    st["base_down"] += delta_down
    st["last_up"] = cur_up
    st["last_down"] = cur_down
    db[loc_id] = st
    save_db(db)
    print(json.dumps({"upload": st["base_up"], "download": st["base_down"]}))

def cmd_reset(loc_id, cur_up, cur_down):
    # صفر کردن آمار پایدار؛ نقطه شروع را روی مقدار خام فعلی kernel می‌گذاریم
    cur_up = int(cur_up)
    cur_down = int(cur_down)
    db = load_db()
    db[loc_id] = {"base_up": 0, "base_down": 0, "last_up": cur_up, "last_down": cur_down}
    save_db(db)
    print("OK")

def cmd_remove(loc_id):
    db = load_db()
    if loc_id in db:
        del db[loc_id]
        save_db(db)
    print("OK")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: traffic_helper.py <update|reset|remove> [args]", file=sys.stderr)
        sys.exit(1)
    cmd = sys.argv[1]
    args = sys.argv[2:]
    try:
        if cmd == "update":
            cmd_update(*args)
        elif cmd == "reset":
            cmd_reset(*args)
        elif cmd == "remove":
            cmd_remove(*args)
        else:
            print(f"ERROR: Unknown command '{cmd}'", file=sys.stderr)
            sys.exit(1)
    except TypeError:
        print("ERROR: Invalid arguments", file=sys.stderr)
        sys.exit(1)
TPYEOF

# 7. ساخت systemd template برای هر لوکیشن
echo -e "${CYAN}[6/8] Creating systemd template unit...${NC}"
cat > /etc/systemd/system/psiphon@.service << 'UNITEOF'
[Unit]
Description=Psiphon Tunnel Instance %i
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/etc/vpn-manager/data/%i
ExecStart=/usr/local/bin/psiphon -config /etc/vpn-manager/configs/%i.json
Restart=always
RestartSec=3

[Install]
WantedBy=multi-user.target
UNITEOF

# 8. ساخت دستور تعاملی vpn-menu (هم منوی ترمینال هم بک‌اند دستورات ربات)
echo -e "${CYAN}[7/8] Installing 'vpn-menu' command...${NC}"
cat > /usr/local/bin/vpn-menu << 'MENUEOF'
#!/bin/bash

GREEN='\033[0;32m'; CYAN='\033[0;36m'; RED='\033[0;31m'; YELLOW='\033[1;33m'; NC='\033[0m'

if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}This command must be run as root. Try: sudo vpn-menu${NC}"
    exit 1
fi

BASE_DIR="/etc/vpn-manager"
CONF_DIR="$BASE_DIR/configs"
DATA_DIR="$BASE_DIR/data"
HELPER="$BASE_DIR/helper.py"
TRAFFIC_HELPER="$BASE_DIR/traffic_helper.py"
SIG_KEY_FILE="$BASE_DIR/sig.key"

if [ -f "$SIG_KEY_FILE" ]; then SIG_KEY=$(cat "$SIG_KEY_FILE"); fi

# ---------- توابع اصلی (منبع مشترک برای منو و ربات تلگرام) ----------

make_config() {
    local id="$1" region="$2" port="$3"
    mkdir -p "$DATA_DIR/$id"
    cat > "$CONF_DIR/$id.json" << EOF
{
    "LocalSocksProxyPort": $port,
    "EgressRegion": "$region",
    "PropagationChannelId": "FFFFFFFFFFFFFFFF",
    "SponsorId": "FFFFFFFFFFFFFFFF",
    "RemoteServerListUrl": "https://s3.amazonaws.com//psiphon/web/mjr4-p23r-puwl/server_list_compressed",
    "RemoteServerListSignaturePublicKey": "$SIG_KEY",
    "RemoteServerListDownloadFilename": "$DATA_DIR/$id/remote_server_list",
    "FetchRemoteServerListRetryIntervalMilliseconds": 1000,
    "DataStoreDirectory": "$DATA_DIR/$id"
}
EOF
}

port_in_use_by_system() {
    local port="$1"
    (command -v ss >/dev/null 2>&1 && ss -ltn 2>/dev/null | awk '{print $4}' | grep -q ":$port$") 
}

# ---------- شمارنده ترافیک (upload/download) هر لوکیشن با iptables ----------

setup_traffic_accounting() {
    local id="$1" port="$2"
    iptables -C INPUT -p tcp --dport "$port" -m comment --comment "vpnmgr_${id}_in" -j ACCEPT 2>/dev/null \
        || iptables -I INPUT -p tcp --dport "$port" -m comment --comment "vpnmgr_${id}_in" -j ACCEPT 2>/dev/null
    iptables -C OUTPUT -p tcp --sport "$port" -m comment --comment "vpnmgr_${id}_out" -j ACCEPT 2>/dev/null \
        || iptables -I OUTPUT -p tcp --sport "$port" -m comment --comment "vpnmgr_${id}_out" -j ACCEPT 2>/dev/null
}

remove_traffic_accounting() {
    local id="$1" port="$2"
    iptables -D INPUT -p tcp --dport "$port" -m comment --comment "vpnmgr_${id}_in" -j ACCEPT 2>/dev/null
    iptables -D OUTPUT -p tcp --sport "$port" -m comment --comment "vpnmgr_${id}_out" -j ACCEPT 2>/dev/null
}

# بازسازی قوانین شمارش ترافیک برای همه لوکیشن‌ها (برای اجرا بعد از بوت سرور، چون iptables پایدار نیست)
ensure_all_traffic_accounting() {
    local ids
    ids=$(python3 "$HELPER" list 2>/dev/null | python3 -c "import json,sys; [print(l['id']) for l in json.load(sys.stdin)]" 2>/dev/null)
    for id in $ids; do
        local info port
        info=$(python3 "$HELPER" get "$id" 2>/dev/null)
        port=$(echo "$info" | python3 -c "import json,sys; print(json.load(sys.stdin).get('port',''))" 2>/dev/null)
        if [ -n "$port" ]; then
            setup_traffic_accounting "$id" "$port"
        fi
    done
}

# اطمینان از وجود قانون شمارش برای یک لوکیشن خاص (محافظه‌کارانه، قبل از start/restart هم صدا زده می‌شود)
ensure_one_traffic_accounting() {
    local id="$1"
    local info port
    info=$(python3 "$HELPER" get "$id" 2>/dev/null) || return 0
    port=$(echo "$info" | python3 -c "import json,sys; print(json.load(sys.stdin).get('port',''))" 2>/dev/null)
    if [ -n "$port" ]; then
        setup_traffic_accounting "$id" "$port"
    fi
}

# چاپ "آپلود دانلود" (بایت خام لحظه‌ای kernel، جدا با فاصله) برای یک لوکیشن
do_traffic_raw() {
    local id="$1"
    local up down
    up=$(iptables -L INPUT -v -n -x 2>/dev/null | grep "vpnmgr_${id}_in" | awk '{print $2}' | head -1)
    down=$(iptables -L OUTPUT -v -n -x 2>/dev/null | grep "vpnmgr_${id}_out" | awk '{print $2}' | head -1)
    up=${up:-0}; down=${down:-0}
    echo "$up $down"
}

# آمار پایدار (بعد از ریبوت سرور هم صفر نمی‌شود) - ترکیب شمارنده خام فعلی با تاریخچه ذخیره‌شده
do_traffic_persistent() {
    local id="$1"
    local cur_up cur_down
    read -r cur_up cur_down <<< "$(do_traffic_raw "$id")"
    python3 "$TRAFFIC_HELPER" update "$id" "$cur_up" "$cur_down" | python3 -c "
import json,sys
d=json.load(sys.stdin)
print(d['upload'], d['download'])
"
}

# صفر کردن دستی آمار ترافیک یک لوکیشن (نقطه شروع جدید از لحظه فعلی)
do_traffic_reset() {
    local id="$1"
    local cur_up cur_down
    read -r cur_up cur_down <<< "$(do_traffic_raw "$id")"
    python3 "$TRAFFIC_HELPER" reset "$id" "$cur_up" "$cur_down"
}

human_readable() {
    python3 -c "
b = float($1)
units = ['B','KB','MB','GB','TB']
i = 0
while b >= 1024 and i < len(units) - 1:
    b /= 1024
    i += 1
print(f'{b:.2f} {units[i]}')
"
}

do_traffic_json() {
    local id="$1"
    local up down total
    read -r up down <<< "$(do_traffic_persistent "$id")"
    total=$((up + down))
    python3 -c "
import json
up=$up; down=$down; total=$total
def h(b):
    units=['B','KB','MB','GB','TB']
    b=float(b); i=0
    while b>=1024 and i<len(units)-1:
        b/=1024; i+=1
    return f'{b:.2f} {units[i]}'
print(json.dumps({'upload':up,'download':down,'total':total,'upload_h':h(up),'download_h':h(down),'total_h':h(total)}))
"
}

do_add() {
    local name="$1" region="$2" port="$3"
    if ! [[ "$port" =~ ^[0-9]+$ ]] || [ "$port" -lt 1 ] || [ "$port" -gt 65535 ]; then
        echo "ERROR: Invalid port '$port'"
        return 1
    fi
    if port_in_use_by_system "$port"; then
        echo "ERROR: Port $port is already in use on this system"
        return 1
    fi
    local result
    result=$(python3 "$HELPER" add "$name" "$region" "$port" 2>&1)
    if [ $? -ne 0 ]; then
        echo "$result"
        return 1
    fi
    local id
    id=$(echo "$result" | python3 -c "import json,sys; print(json.load(sys.stdin)['id'])")
    make_config "$id" "$region" "$port"
    systemctl enable --now "psiphon@$id" >/dev/null 2>&1

    echo "TESTING: checking whether location '$name' (region=$region, port=$port) actually connects..."
    local found_ip=""
    local max_retries=15
    local count=0
    while [ "$count" -lt "$max_retries" ]; do
        found_ip=$(curl -s --socks5 "127.0.0.1:$port" https://api.ipify.org --max-time 3 2>/dev/null)
        if [ -n "$found_ip" ]; then break; fi
        sleep 2
        count=$((count+1))
    done

    if [ -n "$found_ip" ]; then
        setup_traffic_accounting "$id" "$port"
        echo "OK: Location '$name' added as $id (region=$region, port=$port) — connection verified, IP=$found_ip"
        echo "$id"
        return 0
    else
        echo "FAIL: Location '$name' (region=$region, port=$port) is NOT working — no connection after 30s. Rolling back, this location was NOT added."
        systemctl disable --now "psiphon@$id" >/dev/null 2>&1
        rm -f "$CONF_DIR/$id.json"
        rm -rf "$DATA_DIR/$id"
        python3 "$HELPER" remove "$id" >/dev/null 2>&1
        return 1
    fi
}

do_remove() {
    local id="$1"
    local info port
    info=$(python3 "$HELPER" get "$id" 2>/dev/null)
    port=$(echo "$info" | python3 -c "import json,sys; print(json.load(sys.stdin).get('port',''))" 2>/dev/null)
    systemctl disable --now "psiphon@$id" >/dev/null 2>&1
    if [ -n "$port" ]; then
        remove_traffic_accounting "$id" "$port"
    fi
    rm -f "$CONF_DIR/$id.json"
    rm -rf "$DATA_DIR/$id"
    python3 "$TRAFFIC_HELPER" remove "$id" >/dev/null 2>&1
    python3 "$HELPER" remove "$id"
}

do_update_region() {
    local id="$1" new_region="$2"
    local info port old_region
    info=$(python3 "$HELPER" get "$id" 2>&1) || { echo "$info"; return 1; }
    port=$(echo "$info" | python3 -c "import json,sys; print(json.load(sys.stdin)['port'])")
    old_region=$(echo "$info" | python3 -c "import json,sys; print(json.load(sys.stdin)['region'])")

    python3 "$HELPER" update_region "$id" "$new_region" >/dev/null
    make_config "$id" "$new_region" "$port"
    ensure_one_traffic_accounting "$id"
    systemctl restart "psiphon@$id" >/dev/null 2>&1

    echo "TESTING: checking whether region '$new_region' works for location '$id' (port=$port)..."
    local found_ip=""
    local max_retries=15
    local count=0
    while [ "$count" -lt "$max_retries" ]; do
        found_ip=$(curl -s --socks5 "127.0.0.1:$port" https://api.ipify.org --max-time 3 2>/dev/null)
        if [ -n "$found_ip" ]; then break; fi
        sleep 2
        count=$((count+1))
    done

    if [ -n "$found_ip" ]; then
        echo "OK: region changed to '$new_region' — connection verified, IP=$found_ip"
        return 0
    else
        echo "FAIL: region '$new_region' is NOT working — no connection after 30s. Reverting to previous region '$old_region'."
        python3 "$HELPER" update_region "$id" "$old_region" >/dev/null
        make_config "$id" "$old_region" "$port"
        systemctl restart "psiphon@$id" >/dev/null 2>&1
        return 1
    fi
}

do_list_json() {
    python3 "$HELPER" list
}

do_rename() {
    local id="$1" new_name="$2"
    if [ -z "$new_name" ]; then
        echo "ERROR: New name cannot be empty"
        return 1
    fi
    local result
    result=$(python3 "$HELPER" update_name "$id" "$new_name" 2>&1)
    if [ $? -ne 0 ]; then
        echo "$result"
        return 1
    fi
    echo "OK: Location '$id' renamed to '$new_name'"
    return 0
}

do_status_one() {
    local id="$1"
    local status
    status=$(systemctl is-active "psiphon@$id" 2>/dev/null)
    echo "$status"
}

do_ip_check() {
    local port="$1"
    curl -s --socks5 "127.0.0.1:$port" https://api.ipify.org --max-time 5
}

do_uninstall_all() {
    echo "Stopping and removing all locations..."
    local ids
    ids=$(python3 "$HELPER" list 2>/dev/null | python3 -c "import json,sys; [print(l['id']) for l in json.load(sys.stdin)]" 2>/dev/null)
    for id in $ids; do
        echo "  - stopping psiphon@$id"
        local info port
        info=$(python3 "$HELPER" get "$id" 2>/dev/null)
        port=$(echo "$info" | python3 -c "import json,sys; print(json.load(sys.stdin).get('port',''))" 2>/dev/null)
        if [ -n "$port" ]; then
            remove_traffic_accounting "$id" "$port"
        fi
        systemctl disable --now "psiphon@$id" >/dev/null 2>&1
    done
    echo "Stopping Telegram bot service..."
    systemctl disable --now psiphon-bot >/dev/null 2>&1
    echo "Removing traffic-rules boot service..."
    systemctl disable --now vpn-traffic-rules >/dev/null 2>&1
    echo "Removing systemd unit files..."
    rm -f /etc/systemd/system/psiphon@.service
    rm -f /etc/systemd/system/psiphon-bot.service
    rm -f /etc/systemd/system/vpn-traffic-rules.service
    systemctl daemon-reload
    echo "Removing config and data directory ($BASE_DIR)..."
    rm -rf "$BASE_DIR"
    echo "Removing psiphon binary..."
    rm -f /usr/local/bin/psiphon
    echo "Removing Telegram bot script..."
    rm -f /root/manager_bot.py
    echo "Uninstall complete."
    rm -f -- "$0" 2>/dev/null
}

# ---------- حالت غیرتعاملی (برای فراخوانی از ربات تلگرام) ----------

if [ "$1" == "add" ]; then
    shift; do_add "$1" "$2" "$3"; exit $?
elif [ "$1" == "remove" ]; then
    shift; do_remove "$1"; exit $?
elif [ "$1" == "update-region" ]; then
    shift; do_update_region "$1" "$2"; exit $?
elif [ "$1" == "rename" ]; then
    shift; do_rename "$1" "$2"; exit $?
elif [ "$1" == "list" ]; then
    do_list_json; exit 0
elif [ "$1" == "status" ]; then
    shift; do_status_one "$1"; exit 0
elif [ "$1" == "traffic" ]; then
    shift; do_traffic_json "$1"; exit 0
elif [ "$1" == "resettraffic" ]; then
    shift; do_traffic_reset "$1"; exit $?
elif [ "$1" == "ensureaccounting" ]; then
    ensure_all_traffic_accounting; exit 0
elif [ "$1" == "ip" ]; then
    shift; do_ip_check "$1"; exit 0
elif [ "$1" == "start" ]; then
    shift; ensure_one_traffic_accounting "$1"; systemctl start "psiphon@$1" >/dev/null 2>&1; exit $?
elif [ "$1" == "stop" ]; then
    shift; systemctl stop "psiphon@$1" >/dev/null 2>&1; exit $?
elif [ "$1" == "restart" ]; then
    shift; ensure_one_traffic_accounting "$1"; systemctl restart "psiphon@$1" >/dev/null 2>&1; exit $?
elif [ "$1" == "uninstall" ] && [ "$2" == "--yes" ]; then
    do_uninstall_all; exit 0
fi

# ---------- Interactive terminal menu (English, to avoid UTF-8 issues on minimal terminals) ----------

pause() { read -p "Press Enter to continue..." _; }

# حذف کاراکترهای نامرئی (مثل \r از بعضی کلاینت‌های ترمینال) و فاصله اضافه از ورودی کاربر
sanitize() {
    printf '%s' "$1" | tr -d '\r' | xargs 2>/dev/null
}

print_locations() {
    local data
    data=$(do_list_json)
    echo "$data" | python3 -c "
import json, sys
locs = json.load(sys.stdin)
if not locs:
    print('  No locations registered yet.')
for l in locs:
    print(f\"  [{l['id']}] {l['name']}  |  Region: {l['region']}  |  Port: {l['port']}\")
"
}

menu_add() {
    clear
    echo -e "${CYAN}=== Add New Location ===${NC}"
    read -p "Location name: " name
    read -p "Region code (e.g. US, DE, GB): " region
    region=$(sanitize "${region:-US}"); region=${region^^}
    read -p "Dedicated SOCKS port (e.g. 2081): " port
    name=$(sanitize "$name"); port=$(sanitize "$port")
    do_add "$name" "$region" "$port"
    pause
}

menu_remove() {
    clear
    echo -e "${CYAN}=== Remove Location ===${NC}"
    print_locations
    echo ""
    read -p "Location ID to remove (e.g. loc1): " id
    id=$(sanitize "$id")
    [ -z "$id" ] && return
    do_remove "$id"
    pause
}

menu_change_region() {
    clear
    echo -e "${CYAN}=== Change Region of a Location ===${NC}"
    print_locations
    echo ""
    read -p "Location ID: " id
    id=$(sanitize "$id")
    [ -z "$id" ] && return
    read -p "New region code: " region
    region=$(sanitize "$region"); region=${region^^}
    do_update_region "$id" "$region"
    pause
}

menu_rename() {
    clear
    echo -e "${CYAN}=== Rename a Location ===${NC}"
    print_locations
    echo ""
    read -p "Location ID: " id
    id=$(sanitize "$id")
    [ -z "$id" ] && return
    read -p "New name: " new_name
    new_name=$(sanitize "$new_name")
    if [ -z "$new_name" ]; then
        echo -e "${RED}Name cannot be empty.${NC}"
        pause
        return
    fi
    do_rename "$id" "$new_name"
    pause
}

menu_control() {
    clear
    echo -e "${CYAN}=== Start / Stop / Restart a Location ===${NC}"
    print_locations
    echo ""
    read -p "Location ID: " id
    id=$(sanitize "$id")
    if [ -z "$id" ]; then return; fi
    if ! python3 "$HELPER" get "$id" >/dev/null 2>&1; then
        echo -e "${RED}ERROR: Location '$id' not found.${NC}"
        pause
        return
    fi
    echo "1) Start  2) Stop  3) Restart"
    read -p "Choice: " op
    op=$(sanitize "$op")
    case "$op" in
        1)
            ensure_one_traffic_accounting "$id"
            if systemctl start "psiphon@$id"; then
                echo -e "${GREEN}OK: psiphon@$id started.${NC}"
            else
                echo -e "${RED}ERROR: failed to start psiphon@$id (see message above).${NC}"
            fi
            ;;
        2)
            if systemctl stop "psiphon@$id"; then
                echo -e "${GREEN}OK: psiphon@$id stopped.${NC}"
            else
                echo -e "${RED}ERROR: failed to stop psiphon@$id (see message above).${NC}"
            fi
            ;;
        3)
            ensure_one_traffic_accounting "$id"
            if systemctl restart "psiphon@$id"; then
                echo -e "${GREEN}OK: psiphon@$id restarted.${NC}"
            else
                echo -e "${RED}ERROR: failed to restart psiphon@$id (see message above).${NC}"
            fi
            ;;
        *)
            echo -e "${RED}Invalid choice: '$op'${NC}"
            ;;
    esac
    pause
}

menu_uninstall() {
    clear
    echo -e "${RED}=== Completely Remove Service & Script ===${NC}"
    echo -e "${YELLOW}This will permanently delete all locations, the Telegram bot, and the installed scripts.${NC}"
    read -p "Type YES to confirm: " confirm
    confirm=$(sanitize "$confirm")
    if [ "$confirm" == "YES" ]; then
        do_uninstall_all
        exit 0
    else
        echo "Cancelled."
        pause
    fi
}

menu_status_traffic() {
    clear
    echo -e "${CYAN}=== Location Status & Traffic Usage ===${NC}"
    local ids
    ids=$(python3 "$HELPER" list | python3 -c "import json,sys; [print(l['id']) for l in json.load(sys.stdin)]")
    if [ -z "$ids" ]; then
        echo "No locations registered yet."
        pause
        return
    fi
    for id in $ids; do
        local info name region port active up down total up_h down_h total_h icon
        info=$(python3 "$HELPER" get "$id" 2>/dev/null)
        name=$(echo "$info" | python3 -c "import json,sys;print(json.load(sys.stdin)['name'])")
        region=$(echo "$info" | python3 -c "import json,sys;print(json.load(sys.stdin)['region'])")
        port=$(echo "$info" | python3 -c "import json,sys;print(json.load(sys.stdin)['port'])")
        active=$(systemctl is-active "psiphon@$id" 2>/dev/null)
        read -r up down <<< "$(do_traffic_persistent "$id")"
        total=$((up + down))
        up_h=$(human_readable "$up")
        down_h=$(human_readable "$down")
        total_h=$(human_readable "$total")
        icon="🔴"
        [ "$active" == "active" ] && icon="✅"
        echo -e "${icon} [$id] $name  |  Region: $region  |  Port: $port  |  Status: $active"
        echo -e "     Upload: ${GREEN}$up_h${NC}   Download: ${GREEN}$down_h${NC}   Total: ${YELLOW}$total_h${NC}"
        echo ""
    done
    pause
}

menu_reset_traffic() {
    clear
    echo -e "${CYAN}=== Reset Traffic Counter for a Location ===${NC}"
    print_locations
    echo ""
    read -p "Location ID to reset traffic for: " id
    id=$(sanitize "$id")
    if [ -z "$id" ]; then return; fi
    if ! python3 "$HELPER" get "$id" >/dev/null 2>&1; then
        echo -e "${RED}ERROR: Location '$id' not found.${NC}"
        pause
        return
    fi
    read -p "This will reset upload/download stats for '$id' to zero. Type YES to confirm: " confirm
    confirm=$(sanitize "$confirm")
    if [ "$confirm" == "YES" ]; then
        do_traffic_reset "$id"
        echo -e "${GREEN}OK: traffic stats for '$id' have been reset to zero.${NC}"
    else
        echo "Cancelled."
    fi
    pause
}


while true; do
    clear
    echo -e "${CYAN}***************************************************${NC}"
    echo -e "${CYAN}*        MAHDI - VPN MANAGER (vpn-menu)      *${NC}"
    echo -e "${CYAN}***************************************************${NC}"
    echo ""
    echo -e "${GREEN}Current locations:${NC}"
    print_locations
    echo ""
    echo "1) Add new location"
    echo "2) Remove a location"
    echo "3) Change region of a location"
    echo "4) Rename a location"
    echo "5) Start / Stop / Restart a location"
    echo "6) Show status & traffic usage"
    echo "7) Reset traffic counter for a location"
    echo "8) Completely remove service & script from server"
    echo "0) Exit"
    echo ""
    read -p "Your choice: " choice
    case "$choice" in
        1) menu_add ;;
        2) menu_remove ;;
        3) menu_change_region ;;
        4) menu_rename ;;
        5) menu_control ;;
        6) menu_status_traffic ;;
        7) menu_reset_traffic ;;
        8) menu_uninstall ;;
        0) exit 0 ;;
        *) ;;
    esac
done
MENUEOF
chmod +x /usr/local/bin/vpn-menu

# 9. ساخت ربات تلگرام (پشتیبانی از چند لوکیشن + حذف کامل)
echo -e "${CYAN}[8/8] Creating Bot & default location...${NC}"
cat > /root/manager_bot.py << EOF
import telebot
import subprocess
import json
import time
import os
import requests
from telebot import types

# --- Config for MAHDI ---
BOT_TOKEN = "${BOT_TOKEN}"
ADMIN_ID = ${ADMIN_ID}
# ------------------------------

VPN_MENU = "/usr/local/bin/vpn-menu"

bot = telebot.TeleBot(BOT_TOKEN)

ALL_REGIONS = {
    "🇺🇸 US - آمریکا": "US", "🇩🇪 DE - آلمان": "DE", "🇬🇧 GB - انگلیس": "GB",
    "🇫🇷 FR - فرانسه": "FR", "🇨🇦 CA - کانادا": "CA", "🇳🇱 NL - هلند": "NL",
    "🇨🇭 CH - سوئیس": "CH", "🇸🇪 SE - سوئد": "SE", "🇫🇮 FI - فنلاند": "FI",
    "🇦🇹 AT - اتریش": "AT", "🇧🇪 BE - بلژیک": "BE", "🇩🇰 DK - دانمارک": "DK",
    "🇪🇸 ES - اسپانیا": "ES", "🇮🇹 IT - ایتالیا": "IT", "🇮🇪 IE - ایرلند": "IE",
    "🇳🇴 NO - نروژ": "NO", "🇵🇱 PL - لهستان": "PL", "🇹🇷 TR - ترکیه": "TR",
    "🇧🇬 BG - بلغارستان": "BG", "🇨🇿 CZ - چک": "CZ", "🇪🇪 EE - استونی": "EE",
    "🇭🇷 HR - کرواسی": "HR", "🇭🇺 HU - مجارستان": "HU", "🇮🇳 IN - هند": "IN",
    "🇯🇵 JP - ژاپن": "JP", "🇱🇻 LV - لتونی": "LV", "🇵🇹 PT - پرتغال": "PT",
    "🇷🇴 RO - رومانی": "RO", "🇷🇸 RS - صربستان": "RS", "🇸🇬 SG - سنگاپور": "SG",
    "🇸🇰 SK - اسلواکی": "SK"
}

pending_action = {}  # user_id -> {"action": ..., "data": {...}}

def is_admin(user_id):
    return user_id == ADMIN_ID

def run(args):
    try:
        result = subprocess.run([VPN_MENU] + args, capture_output=True, text=True, timeout=45)
        return result.returncode, result.stdout.strip(), result.stderr.strip()
    except Exception as e:
        return 1, "", str(e)

def get_locations():
    code, out, err = run(["list"])
    if code != 0 or not out:
        return []
    try:
        return json.loads(out)
    except Exception:
        return []

def main_menu():
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    markup.add("📊 وضعیت لوکیشن‌ها", "➕ افزودن لوکیشن")
    markup.add("🗑 حذف لوکیشن", "🌍 تغییر ریجن لوکیشن")
    markup.add("✏️ تغییر نام لوکیشن", "🧹 صفر کردن ترافیک")
    markup.add("▶️ استارت", "⏹ استاپ", "🔄 ریستارت")
    markup.add("❌ حذف کامل سرویس از سرور")
    return markup

def region_keyboard(prefix):
    markup = types.InlineKeyboardMarkup(row_width=3)
    buttons = [types.InlineKeyboardButton(name, callback_data=f"{prefix}_{code}") for name, code in ALL_REGIONS.items()]
    markup.add(*buttons)
    return markup

def locations_keyboard(prefix):
    locs = get_locations()
    markup = types.InlineKeyboardMarkup(row_width=1)
    for loc in locs:
        label = f"{loc['name']} ({loc['region']}:{loc['port']})"
        markup.add(types.InlineKeyboardButton(label, callback_data=f"{prefix}_{loc['id']}"))
    return markup, locs

@bot.message_handler(commands=['start', 'help'])
def send_welcome(message):
    if is_admin(message.from_user.id):
        bot.reply_to(message, "👋 سلام قربان!\n🌹 به پنل مدیریت اختصاصی **مهدی** خوش آمدید.", reply_markup=main_menu(), parse_mode="Markdown")
    else:
        bot.reply_to(message, "⛔️ دسترسی غیرمجاز است.")

@bot.message_handler(func=lambda m: True)
def handle_messages(message):
    if not is_admin(message.from_user.id):
        return
    msg = message.text
    cid = message.chat.id
    uid = message.from_user.id

    if msg == "📊 وضعیت لوکیشن‌ها":
        locs = get_locations()
        if not locs:
            bot.send_message(cid, "هیچ لوکیشنی ثبت نشده است.")
            return
        bot.send_message(cid, "⏳ در حال بررسی وضعیت و ترافیک...")
        lines = []
        for loc in locs:
            code, status, _ = run(["status", loc["id"]])
            icon = "✅" if status == "active" else "🔴"
            traffic_line = ""
            tcode, tout, _ = run(["traffic", loc["id"]])
            if tcode == 0 and tout:
                try:
                    t = json.loads(tout)
                    traffic_line = (
                        f"\n   آپلود: {t['upload_h']} ⬆️"
                        f"\n   دانلود: {t['download_h']} ⬇️"
                        f"\n   جمع کل: {t['total_h']}"
                    )
                except Exception:
                    pass
            block = (
                f"{icon} **{loc['name']}** [{loc['id']}]\n"
                f"   ریجن: {loc['region']}\n"
                f"   پورت: {loc['port']}\n"
                f"   وضعیت: {status}"
                f"{traffic_line}"
            )
            lines.append(block)
        bot.send_message(cid, "\n\n".join(lines), parse_mode="Markdown")

    elif msg == "➕ افزودن لوکیشن":
        pending_action[uid] = {"action": "add", "data": {}}
        bot.send_message(cid, "نام دلخواه برای این لوکیشن را وارد کنید:")
        bot.register_next_step_handler(message, add_step_name)

    elif msg == "🗑 حذف لوکیشن":
        markup, locs = locations_keyboard("delloc")
        if not locs:
            bot.send_message(cid, "هیچ لوکیشنی برای حذف وجود ندارد.")
            return
        bot.send_message(cid, "لوکیشن مورد نظر برای حذف را انتخاب کنید:", reply_markup=markup)

    elif msg == "🌍 تغییر ریجن لوکیشن":
        markup, locs = locations_keyboard("pickregionloc")
        if not locs:
            bot.send_message(cid, "هیچ لوکیشنی وجود ندارد.")
            return
        bot.send_message(cid, "ابتدا لوکیشن را انتخاب کنید:", reply_markup=markup)

    elif msg == "✏️ تغییر نام لوکیشن":
        markup, locs = locations_keyboard("pickrenameloc")
        if not locs:
            bot.send_message(cid, "هیچ لوکیشنی وجود ندارد.")
            return
        bot.send_message(cid, "لوکیشنی که می‌خواهید نامش را تغییر دهید انتخاب کنید:", reply_markup=markup)

    elif msg == "🧹 صفر کردن ترافیک":
        markup, locs = locations_keyboard("pickresettraffic")
        if not locs:
            bot.send_message(cid, "هیچ لوکیشنی وجود ندارد.")
            return
        bot.send_message(cid, "لوکیشنی که می‌خواهید آمار ترافیکش صفر شود را انتخاب کنید:", reply_markup=markup)

    elif msg in ("▶️ استارت", "⏹ استاپ", "🔄 ریستارت"):
        action_map = {"▶️ استارت": "start", "⏹ استاپ": "stop", "🔄 ریستارت": "restart"}
        prefix = "svc_" + action_map[msg]
        markup, locs = locations_keyboard(prefix)
        if not locs:
            bot.send_message(cid, "هیچ لوکیشنی وجود ندارد.")
            return
        bot.send_message(cid, "لوکیشن مورد نظر را انتخاب کنید:", reply_markup=markup)

    elif msg == "❌ حذف کامل سرویس از سرور":
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("⚠️ بله، همه چیز حذف شود", callback_data="confirm_full_uninstall"))
        markup.add(types.InlineKeyboardButton("انصراف", callback_data="cancel_full_uninstall"))
        bot.send_message(cid, "🚨 این کار تمام لوکیشن‌ها، ربات و اسکریپت‌های نصب‌شده را برای همیشه از سرور حذف می‌کند.\nآیا مطمئن هستید؟", reply_markup=markup)

def add_step_name(message):
    if not is_admin(message.from_user.id):
        return
    uid = message.from_user.id
    pending_action[uid]["data"]["name"] = message.text.strip()
    bot.send_message(message.chat.id, "ریجن را انتخاب کنید:", reply_markup=region_keyboard("addregion"))

def add_step_port(message):
    if not is_admin(message.from_user.id):
        return
    uid = message.from_user.id
    cid = message.chat.id
    port_text = message.text.strip()
    if not port_text.isdigit():
        bot.send_message(cid, "پورت باید عدد باشد. دوباره تلاش کنید یا /start را بزنید.")
        return
    data = pending_action.get(uid, {}).get("data", {})
    name = data.get("name")
    region = data.get("region")
    bot.send_message(cid, "⏳ در حال ساخت لوکیشن و تست اتصال (ممکن است تا ۳۰ ثانیه طول بکشد)...")
    code, out, err = run(["add", name, region, port_text])
    if code == 0:
        # استخراج آی‌پی از خروجی برای نمایش به کاربر
        ip_part = ""
        if "IP=" in out:
            ip_part = f"\n🌐 آی‌پی: {out.split('IP=')[-1].splitlines()[0].strip()}"
        bot.send_message(cid, f"✅ لوکیشن **{name}** با ریجن {region} روی پورت {port_text} اضافه شد و اتصالش تایید شد.{ip_part}", parse_mode="Markdown", reply_markup=main_menu())
    elif "FAIL:" in out:
        bot.send_message(cid, f"⚠️ این لوکیشن کار نمی‌کند!\nریجن **{region}** روی پورت {port_text} بعد از ۳۰ ثانیه اتصال برقرار نکرد و اضافه نشد.\nمی‌توانید ریجن یا پورت دیگری امتحان کنید.", parse_mode="Markdown", reply_markup=main_menu())
    else:
        bot.send_message(cid, f"❌ خطا: {out}\n{err}", reply_markup=main_menu())
    pending_action.pop(uid, None)

@bot.callback_query_handler(func=lambda call: call.data.startswith("addregion_"))
def cb_add_region(call):
    if not is_admin(call.from_user.id):
        return
    uid = call.from_user.id
    region = call.data.split("_", 1)[1]
    pending_action.setdefault(uid, {"action": "add", "data": {}})
    pending_action[uid]["data"]["region"] = region
    bot.answer_callback_query(call.id)
    bot.edit_message_text(f"ریجن انتخابی: {region}", call.message.chat.id, call.message.message_id)
    msg = bot.send_message(call.message.chat.id, "پورت اختصاصی این لوکیشن را وارد کنید (مثلا 2081):")
    bot.register_next_step_handler(msg, add_step_port)

@bot.callback_query_handler(func=lambda call: call.data.startswith("delloc_"))
def cb_delete_location(call):
    if not is_admin(call.from_user.id):
        return
    loc_id = call.data.split("_", 1)[1]
    cid = call.message.chat.id
    bot.answer_callback_query(call.id, "در حال حذف...")
    code, out, err = run(["remove", loc_id])
    if code == 0:
        bot.edit_message_text(f"✅ لوکیشن {loc_id} حذف شد.", cid, call.message.message_id)
    else:
        bot.edit_message_text(f"❌ خطا در حذف: {out} {err}", cid, call.message.message_id)

@bot.callback_query_handler(func=lambda call: call.data.startswith("pickregionloc_"))
def cb_pick_region_loc(call):
    if not is_admin(call.from_user.id):
        return
    loc_id = call.data.split("_", 1)[1]
    cid = call.message.chat.id
    bot.answer_callback_query(call.id)
    pending_action[call.from_user.id] = {"action": "region", "data": {"loc_id": loc_id}}
    bot.edit_message_text("ریجن جدید را انتخاب کنید:", cid, call.message.message_id, reply_markup=region_keyboard("applyregion"))

@bot.callback_query_handler(func=lambda call: call.data.startswith("pickrenameloc_"))
def cb_pick_rename_loc(call):
    if not is_admin(call.from_user.id):
        return
    loc_id = call.data.split("_", 1)[1]
    cid = call.message.chat.id
    bot.answer_callback_query(call.id)
    pending_action[call.from_user.id] = {"action": "rename", "data": {"loc_id": loc_id}}
    msg = bot.send_message(cid, "نام جدید برای این لوکیشن را وارد کنید:")
    bot.register_next_step_handler(msg, rename_step_name)

def rename_step_name(message):
    if not is_admin(message.from_user.id):
        return
    uid = message.from_user.id
    cid = message.chat.id
    new_name = message.text.strip()
    loc_id = pending_action.get(uid, {}).get("data", {}).get("loc_id")
    if not loc_id:
        return
    if not new_name:
        bot.send_message(cid, "نام نمی‌تواند خالی باشد. دوباره تلاش کنید یا /start را بزنید.")
        pending_action.pop(uid, None)
        return
    code, out, err = run(["rename", loc_id, new_name])
    if code == 0:
        bot.send_message(cid, f"✅ نام لوکیشن {loc_id} به **{new_name}** تغییر یافت.", parse_mode="Markdown", reply_markup=main_menu())
    else:
        bot.send_message(cid, f"❌ خطا: {out}\n{err}", reply_markup=main_menu())
    pending_action.pop(uid, None)

@bot.callback_query_handler(func=lambda call: call.data.startswith("pickresettraffic_"))
def cb_pick_reset_traffic(call):
    if not is_admin(call.from_user.id):
        return
    loc_id = call.data.split("_", 1)[1]
    cid = call.message.chat.id
    bot.answer_callback_query(call.id)
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("⚠️ بله، صفر شود", callback_data=f"confirmresettraffic_{loc_id}"))
    markup.add(types.InlineKeyboardButton("انصراف", callback_data="cancel_reset_traffic"))
    bot.edit_message_text(f"آمار آپلود/دانلود لوکیشن {loc_id} صفر می‌شود. مطمئن هستید؟", cid, call.message.message_id, reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "cancel_reset_traffic")
def cb_cancel_reset_traffic(call):
    if not is_admin(call.from_user.id):
        return
    bot.answer_callback_query(call.id, "لغو شد")
    bot.edit_message_text("عملیات صفر کردن ترافیک لغو شد.", call.message.chat.id, call.message.message_id)

@bot.callback_query_handler(func=lambda call: call.data.startswith("confirmresettraffic_"))
def cb_confirm_reset_traffic(call):
    if not is_admin(call.from_user.id):
        return
    loc_id = call.data.split("_", 1)[1]
    cid = call.message.chat.id
    bot.answer_callback_query(call.id, "در حال صفر کردن...")
    code, out, err = run(["resettraffic", loc_id])
    if code == 0:
        bot.edit_message_text(f"✅ آمار ترافیک لوکیشن {loc_id} صفر شد.", cid, call.message.message_id)
    else:
        bot.edit_message_text(f"❌ خطا: {out} {err}", cid, call.message.message_id)

@bot.callback_query_handler(func=lambda call: call.data.startswith("applyregion_"))
def cb_apply_region(call):
    if not is_admin(call.from_user.id):
        return
    uid = call.from_user.id
    region = call.data.split("_", 1)[1]
    cid = call.message.chat.id
    loc_id = pending_action.get(uid, {}).get("data", {}).get("loc_id")
    if not loc_id:
        return
    bot.answer_callback_query(call.id, "در حال اعمال...")
    bot.edit_message_text(f"⏳ تست اتصال با ریجن {region} برای {loc_id} (تا ۳۰ ثانیه طول می‌کشد)...", cid, call.message.message_id)
    code, out, err = run(["update-region", loc_id, region])
    if code == 0:
        ip_part = ""
        if "IP=" in out:
            ip_part = f"\n🌐 آی‌پی: {out.split('IP=')[-1].splitlines()[0].strip()}"
        bot.edit_message_text(f"✅ ریجن لوکیشن {loc_id} به {region} تغییر یافت و اتصالش تایید شد.{ip_part}", cid, call.message.message_id)
    elif "FAIL:" in out:
        bot.edit_message_text(f"⚠️ ریجن {region} برای {loc_id} کار نمی‌کند!\nبعد از ۳۰ ثانیه اتصال برقرار نشد، بنابراین به ریجن قبلی (که کار می‌کرد) برگردانده شد و تغییری اعمال نشد.", cid, call.message.message_id)
    else:
        bot.edit_message_text(f"❌ خطا: {out} {err}", cid, call.message.message_id)
    pending_action.pop(uid, None)

@bot.callback_query_handler(func=lambda call: call.data.startswith("svc_"))
def cb_service_control(call):
    if not is_admin(call.from_user.id):
        return
    _, action, loc_id = call.data.split("_", 2)
    cid = call.message.chat.id
    bot.answer_callback_query(call.id, "در حال اجرا...")
    code, out, err = run([action, loc_id])
    labels = {"start": "استارت", "stop": "استاپ", "restart": "ریستارت"}
    if code == 0:
        bot.edit_message_text(f"✅ عملیات {labels.get(action, action)} برای {loc_id} انجام شد.", cid, call.message.message_id)
    else:
        bot.edit_message_text(f"❌ خطا: {out} {err}", cid, call.message.message_id)

@bot.callback_query_handler(func=lambda call: call.data == "cancel_full_uninstall")
def cb_cancel_uninstall(call):
    if not is_admin(call.from_user.id):
        return
    bot.answer_callback_query(call.id, "لغو شد")
    bot.edit_message_text("عملیات حذف لغو شد.", call.message.chat.id, call.message.message_id)

@bot.callback_query_handler(func=lambda call: call.data == "confirm_full_uninstall")
def cb_confirm_uninstall(call):
    if not is_admin(call.from_user.id):
        return
    cid = call.message.chat.id
    bot.answer_callback_query(call.id)
    bot.edit_message_text("🗑 در حال حذف کامل سرویس، لوکیشن‌ها و ربات از سرور...\nاین آخرین پیام ربات خواهد بود.", cid, call.message.message_id)
    # اجرای حذف به صورت جدا از پردازش ربات تا بعد از توقف سرویس ربات هم ادامه پیدا کند
    subprocess.Popen(
        ["nohup", "bash", "-c", "sleep 2; " + VPN_MENU + " uninstall --yes"],
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True
    )
    os._exit(0)

bot.infinity_polling()
EOF

# ساخت سرویس ربات
cat > /etc/systemd/system/psiphon-bot.service << 'EOF'
[Unit]
Description=Telegram Bot for Mahdi Manager
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/root
ExecStart=/usr/bin/python3 /root/manager_bot.py
Restart=always
RestartSec=10
KillMode=process

[Install]
WantedBy=multi-user.target
EOF

# سرویس بازسازی خودکار قوانین شمارش ترافیک بعد از هر بوت سرور
# (چون قوانین iptables پایدار نیستند و با ریبوت سرور از بین می‌روند)
cat > /etc/systemd/system/vpn-traffic-rules.service << 'EOF'
[Unit]
Description=Restore VPN Manager traffic accounting rules after boot
After=network.target

[Service]
Type=oneshot
ExecStart=/usr/local/bin/vpn-menu ensureaccounting
RemainAfterExit=yes

[Install]
WantedBy=multi-user.target
EOF

# راه‌اندازی نهایی
systemctl daemon-reload

# ساخت لوکیشن پیش‌فرض (سازگار با نصب‌های قبلی: US روی پورت 2080)
echo -e "${YELLOW}Testing default location connectivity (up to 30s)...${NC}"
/usr/local/bin/vpn-menu add "Default-US" "US" "2080"

systemctl enable psiphon-bot >/dev/null 2>&1
systemctl restart psiphon-bot

systemctl enable vpn-traffic-rules >/dev/null 2>&1
systemctl restart vpn-traffic-rules >/dev/null 2>&1

echo ""
echo -e "${GREEN}**************************************************${NC}"
echo -e "${GREEN}* INSTALLATION COMPLETE! 🎉              *${NC}"
echo -e "${GREEN}* Designed for: MAHDI                 *${NC}"
echo -e "${GREEN}**************************************************${NC}"
echo -e "1. Default Location: US on port 2080"
echo -e "2. Bot Status: STARTED"
echo -e "3. Terminal Command: type 'vpn-menu' to manage locations or uninstall"
echo -e "4. Add more locations from the terminal menu or directly from the Telegram bot"
echo ""
